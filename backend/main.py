...
@app.get('/all_business_products')
async def all_business_product(session: Session = Depends(get_session)):
    return session.query(models.BusinessProduct).all()
...
async def create_blog(blog: schema.Blog, session: Session = Depends(get_session)):
    new_user = crud.Creat_blog(blog, session)
    return {'User': new_user}


@app.post('/create_user')
async def create_user(user: schema.User, session: Session = Depends(get_session)):
    new = crud.Create_user(user, session)
    return {'ser': new}


@app.post('/create_author')
async def create_author(author: schema.Author, session: Session = Depends(get_session)):
    auth = crud.create_autor(author, session)
    return {"author": auth}
...
async def add_product_image(pi: schema.ProductImage, session: Session = Depends(get_session)):
    image = crud.product_image(pi, session)

    return {"Image Added": image}
...
async def processing_option(po: schema.ProcessingOption, session: Session = Depends(get_session)):
...
@app.post('/cart')
async def cart(cart: schema.Cart, session: Session = Depends(get_session)):
...
@app.post('/cart_item')
async def cart_item(ci: schema.CartItem, session: Session = Depends(get_session)):
    caart = crud.cart_item(ci, session)

    return {"Cart Item": caart}
...
@app.get("/user/id/1", response_model=schema.User)
...
@app.get("/delivery_address/id/{id}", response_model=schema.DeliveryAddress)
def get_user(id: int, session: Session = Depends(get_session)):
...
@app.get("/order/id/{id}", response_model=schema.Order)
def get_user(id: int, session: Session = Depends(get_session)):
